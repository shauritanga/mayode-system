...
const YIELD_PER_ACRE_KG: Record<FarmGrade, number> = {
  A: 2200,
  B: 1800,
  C: 1400,
};
...
export class FarmReportsService {
  ...
  async deletePhoto(photoId: string, user: RequestUser) {
    const photo = await this.prisma.farmPhoto.findUnique({
      where: { id: photoId },
    });
  ...
  async createFieldSurvey(
    farmId: string,
    dto: CreateFieldSurveyDto,
    user: RequestUser,
  ) {
  ...
  private async assembleReport(farmId: string) {
    ...
    const estimatedValueTzs = Math.round(
      LAND_VALUE_PER_ACRE_TZS[farm.grade] * acres,
    );
    const actualYieldKg = farm.cropCycles.reduce(
    ...
    if (survey?.soilPh != null) {
      if (survey.soilPh < 5.5)
        recommendations.push(
          'Soil is acidic (pH < 5.5). Consider agricultural lime before planting.',
        );
      else if (survey.soilPh > 7.5)
        recommendations.push(
          'Soil is alkaline (pH > 7.5). Use acidifying inputs and monitor micronutrients.',
        );
    } else {
      recommendations.push(
        'No soil test on record. Request a MAYODE field soil assessment.',
      );
    }
    if (survey?.roadAccessQuality === 'POOR')
      recommendations.push(
        'Road access is poor — plan harvest transport early to reduce post-harvest loss.',
      );
    if (!farm.hasIrrigation && !survey?.waterSource)
      recommendations.push(
        'No irrigation or water source recorded — assess water access for the dry season.',
      );
    if (!farm.centerLatitude)
      recommendations.push(
        'Farm is not GPS-mapped yet. Capture the boundary to unlock mapping-based analytics.',
      );
    if (farm.photos.length < 3)
      recommendations.push(
        'Add at least 3 farm photos to complete the farm profile.',
      );

    let condition = 'Fair';
    if (farm.grade === 'A' && farm.hasIrrigation) condition = 'Good';
    else if (farm.grade === 'C' || survey?.floodRisk === 'High')
  ...
  private basicReport(
    r: Awaited<ReturnType<FarmReportsService['assembleReport']>>,
  ) {
    const f = r.farm;
    return {
      farmId: f.id,
      farmCode: f.farmCode,
      name: f.name,
      location: [f.village, f.ward, f.district, f.region, f.mamcos?.name]
        .filter(Boolean)
        .join(', '),
  ...
  async getReport(farmId: string, user: RequestUser) {
    ...
    return {
      locked: false,
      ...basic,
      gps: f.centerLatitude
        ? { latitude: f.centerLatitude, longitude: f.centerLongitude }
        : null,
      ...
      soil: r.survey
        ...
        : f.soilType || f.soilCondition
          ? {
              type: f.soilType,
              condition: f.soilCondition,
              source: 'FARMER_REPORTED',
            }
          : null,
      roadAccess: r.survey
        ? {
            distanceMeters: r.survey.roadDistanceMeters,
            quality: r.survey.roadAccessQuality,
          }
        : { nearRoad: f.nearRoad },
      waterAccess: r.survey
        ? {
            source: r.survey.waterSource,
            distanceMeters: r.survey.waterDistanceMeters,
            reliability: r.survey.waterReliability,
          }
...
<p style="color:#92400e">${esc(r.message)}</p></div></body></html>`;
  ...
  const rows = (r.recommendations ?? [])
    .map((x: string) => `<li>${esc(x)}</li>`)
    .join('');
  const photos = (r.photos ?? [])
    .map(
      (p: any) =>
        `<div style="font-size:12px;color:#6b7280">📷 ${esc(p.caption || p.url)}</div>`,
    )
    .join('');
