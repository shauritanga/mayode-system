...
describe('SalesService buyer collection', () => {
  ...
  it('initiates a buyer collection and saves its order reference', async () => {
    prisma.sale.findUnique.mockResolvedValue({
      ...
      totalRevenue: 100000,
      fairtradePremium: 5000,
      buyer: { contactPhone: '0712345678' },
    });
    prisma.sale.update.mockResolvedValue({
      id: 'abc12345-sale',
      buyerOrderReference: 'SALE-abc12345-1',
    });
    ...
    expect(clickPesa.initiateUssdPush).toHaveBeenCalledWith(
      expect.objectContaining({
        amount: '105000',
        orderReference: 'SALE-abc12345-1',
        phoneNumber: '0712345678',
      }),
    );
    expect(prisma.sale.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ buyerPaymentReference: 'cp-1' }),
      }),
    );
    now.mockRestore();
  });
  ...
  it('settles once when ClickPesa confirms success', async () => {
    ...
    const settle = jest
      ...
      .mockResolvedValue({ id: 'sale-1' } as any);
  ...
  it('refuses collection when ClickPesa is not configured', async () => {
    ...
    await expect(service.collectBuyerPayment('sale-1')).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
});
