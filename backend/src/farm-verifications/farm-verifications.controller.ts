...
export class FarmVerificationsController {
  ...
  @ApiOperation({ summary: 'Submit farm verification proof & neighbor details (Field Officer / Admin only)' })
  verifyFarm(@CurrentUser() user: { id: string }, @Body() verifyFarmDto: VerifyFarmDto) {
    return this.farmVerificationsService.verifyFarm(user.id, verifyFarmDto);
  }

  @Get()
  @ApiOperation({ summary: 'Get all farm verification records across the system' })
  findAll() {
    return this.farmVerificationsService.findAll();
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get farm verification details by verification record ID' })
