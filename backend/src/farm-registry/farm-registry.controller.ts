...
export class FarmRegistryController {
  ...
  @ApiOperation({ summary: 'AMCOS/officer pre-registers a farm under a known owner' })
  preRegister(@Body() dto: PreRegisterFarmDto, @CurrentUser() user: RequestUser) {
  ...
  list(@Query('status') status?: FarmRegistryStatus, @Query('mamcosId') mamcosId?: string) {
    return this.registry.listAll(status, mamcosId);
  }

  @Get('mine')
  @ApiOperation({ summary: 'Pre-registered farms awaiting confirmation from the current owner' })
  ...
  @ApiOperation({ summary: 'Owner confirms & claims a pre-registered farm (materializes a Farm)' })
  ...
  @Post(':id/reject')
  @Roles(UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.FIELD_OFFICER, UserRole.FARMER)
  @ApiOperation({ summary: 'Owner rejects a pre-registered farm as not theirs (marks disputed)' })
